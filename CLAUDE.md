...
gitnexus-cli (binary: "gitnexus")
  ├── gitnexus-mcp        (MCP server: 30 tools, stdio/HTTP transport, JSON-RPC 2.0)
...
**MCP** (`gitnexus-mcp`): Implements MCP protocol version 2024-11-05. Thirty tools dispatched in `backend/local.rs`:
...
- **Doc-SFD authoring (P1.1)**: `list_sfd_pages`, `write_sfd_draft`, `validate_sfd` — backed by `gitnexus-rag::sfd`, shared between the desktop chat (Tauri) and the HTTP MCP transport (chat-ui).
...
- **Enrichment knobs in chat-config.json** (`EnrichmentConfig` in `enrichment.rs`): tune-without-rebuild for the constants that previously forced a `cargo build --release` to change. Add an optional `enrichment` block to `~/.gitnexus/chat-config.json`: `sectionMaxTokens` (default 4096 — per-section LLM call), `monolithicMaxTokensFloor` (default 65_536 — full-page floor), `sectionContentSnippetBytes` (default 3000 — snippet cap), and `profiles.{fast,quality,strict}.{maxEvidence,maxRetries,timeoutSecs,minGapMs,useJsonSchema}` for per-profile overrides. All fields optional; absent block = exact prior behavior. `thinking_boost` and `review_critical` are intentionally NOT exposed — they encode the named profile's intent.
