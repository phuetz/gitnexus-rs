...
pub async fn get_neighbors(
    ...
    if dir == "outgoing" || dir == "both" {
        if let Some(outs) = indexes.outgoing.get(&node_id) {
            for (target, rel_type) in outs {
                ...
                edges.push(CytoEdge {
                    id: format!("{}->{}:{}", node_id, target, rel_type.as_str()),
    ...
    if dir == "incoming" || dir == "both" {
        if let Some(ins) = indexes.incoming.get(&node_id) {
            for (source, rel_type) in ins {
                ...
                edges.push(CytoEdge {
                    id: format!("{}->{}:{}", source, node_id, rel_type.as_str()),
                    source: source.clone(),
    ...
    // Build nodes: include center + neighbors
    let mut nodes = Vec::new();
    if let Some(center) = graph.get_node(&node_id) {
        nodes.push(node_to_cyto(center));
    }
    for id in &neighbor_ids {
        if let Some(n) = graph.get_node(id) {
            nodes.push(node_to_cyto(n));
        }
    }
