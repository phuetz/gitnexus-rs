...
export function brightenColor(hex: string, factor: number): string {
  ...
  const g = parseInt(hex.slice(3, 5), 16);
  const b = parseInt(hex.slice(5, 7), 16);
  const nr = Math.min(255, Math.round(r + (255 - r) * (factor - 1) / factor));
  const ng = Math.min(255, Math.round(g + (255 - g) * (factor - 1) / factor));
  const nb = Math.min(255, Math.round(b + (255 - b) * (factor - 1) / factor));
  return `#${nr.toString(16).padStart(2, "0")}${ng.toString(16).padStart(2, "0")}${nb.toString(16).padStart(2, "0")}`;
}
